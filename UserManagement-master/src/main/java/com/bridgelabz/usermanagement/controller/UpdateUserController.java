...
public class UpdateUserController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (userId > 0) {
            logger.info(" this is Updated User information message");
            try {
                boolean updateUser = updateUserDAO.updateUser(user, imageStream, userId);
                updateUserDAO.updatePermissions(userId, 1, permissions.isDashboardAdd(), permissions.isDashboardDelete(), permissions.isDashboardModify(), permissions.isDashboardRead());
                updateUserDAO.updatePermissions(userId, 2, permissions.isSettingsAdd(), permissions.isSettingsDelete(), permissions.isSettingsModify(), permissions.isSettingsRead());
                updateUserDAO.updatePermissions(userId, 3, permissions.isUserInformationAdd(), permissions.isUserInformationDelete(), permissions.isUserInformationModify(), permissions.isUserInformationRead());
